...
internal sealed class UpdaterPrestageSession : IDisposable
{
    ...
    public async Task FinishAsync(IProgress<UpdateProgress>? progress)
    {
        try
        {
            if (!_work.IsCompleted)
            {
                ...
                await Task.WhenAny(_work, Task.Delay(Grace));
            ...
            if (!_work.IsCompleted)
            {
                ...
                _log("Updater pre-staging: download continues on the next launch.");
                return;
            }
            PrestagePreparation? preparation = await _work;
            if (preparation is null)
            {
                return;
            }
            _log($"Updater pre-staging: {preparation.Summary}.");
            if (preparation.Ready is not null)
            {
                UpdaterPrestageSwap.StartHelper(_layout, _ownExe, _self, _lockFile, preparation.Ready, _log);
            }
        }
        catch (Exception exception)
        {
            _log($"Updater pre-staging could not start its swap helper ({exception.GetType().Name}: {exception.Message}).");
        }
    }
